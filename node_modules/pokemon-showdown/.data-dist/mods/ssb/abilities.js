	...
	*/
	...
	lastminutelag: {
		...
		onModifyMove(move, pokemon, target) {
			if (_optionalChain([move, 'access', _27 => _27.self, 'optionalAccess', _28 => _28.volatileStatus]) === 'mustrecharge') {
	...
	plausibledeniability: {
		...
		onModifyPriority(priority, pokemon, target, move) {
			if (_optionalChain([move, 'optionalAccess', _29 => _29.category]) === 'Status') {
	...
	shadydeal: {
		...
		onTryHit(target, source, move) {
			if (target !== source && _optionalChain([this, 'access', _30 => _30.effectState, 'access', _31 => _31.immunities, 'optionalAccess', _32 => _32.includes, 'call', _33 => _33(move.type)])) {
	...
	combattraining: {
		...
		onDamage(damage, target, source, effect) {
			...
			if (
				_optionalChain([effect, 'optionalAccess', _34 => _34.effectType]) === 'Move' &&
		...
		onEnd(pokemon) {
			if (pokemon.illusion) {
				...
				if (!pokemon.volatiles[this.toID(pokemon.name)]) {
					...
					if (_optionalChain([status, 'optionalAccess', _35 => _35.exists])) {
	...
	stakeout: {
		...
		onModifyAtk(atk, attacker, defender) {
			if (!_optionalChain([defender, 'optionalAccess', _36 => _36.activeTurns])) {
