	...
	randomSet(
		...
		do {
			...
			const runEnforcementChecker = (checkerName) => (
				_optionalChain([this, 'access', _ => _.moveEnforcementCheckers, 'access', _2 => _2[checkerName], 'optionalCall', _3 => _3(
	...
	randomBSSFactoryTeam(side, depth = 0) {
		...
		while (pokemonPool.length && pokemon.length < this.maxTeamSize) {
			...
			for (const typeName of this.dex.types.names()) {
				// Cover any major weakness (3+) with at least one resistance
				if (teamData.resistances[typeName] >= 1) continue;
				if (_optionalChain([resistanceAbilities, 'access', _7 => _7[abilityState.id], 'optionalAccess', _8 => _8.includes, 'call', _9 => _9(typeName)]) || !this.dex.getImmunity(typeName, types)) {
					// Heuristic: assume that Pokémon with these abilities don't have (too) negative typing.
					teamData.resistances[typeName] = (teamData.resistances[typeName] || 0) + 1;
					if (teamData.resistances[typeName] >= 1) teamData.weaknesses[typeName] = 0;
					continue;
