...
function SSBSets(target) {
 ...
 const commands = {
	...
	randombattles(target, room, user) {
		...
		} else if (_optionalChain([room, 'optionalAccess', _17 => _17.battle])) {
	...
	randomdoublesbattle(target, room, user) {
		...
		if (args[1] && toID(args[1]) in Dex.dexes) {
			dex = Dex.dexes[toID(args[1])];
		} else if (_optionalChain([room, 'optionalAccess', _18 => _18.battle])) {
			const format = Dex.formats.get(room.battle.format);
			dex = Dex.mod(format.mod);
		}
		if (parseInt(toID(args[1])[3]) < 4) {
			if (_optionalChain([room, 'optionalAccess', _19 => _19.battle])) {
				const format = Dex.formats.get(room.battle.format);
				dex = Dex.mod(format.mod);
			} else {
