 ...
 */
 ...
 const commands = {
	...
	promote(target, room, user, connection, cmd) {
		...
		const userid = toID(targetUsername);
		const name = _optionalChain([targetUser, 'optionalAccess', _51 => _51.name]) || targetUsername;

		if (!userid) return this.parse('/help promote');

		const currentGroup = _optionalChain([targetUser, 'optionalAccess', _52 => _52.tempGroup]) || Users.globalAuth.get(userid);
	...
	trustuser(target, room, user, connection, cmd) {
		...
		if (rest) return this.errorReply(`This command does not support specifying a reason.`);
		const userid = toID(targetUsername);
		const name = _optionalChain([targetUser, 'optionalAccess', _53 => _53.name]) || targetUsername;
	...
	sectionleader(target, room, user, connection, cmd) {
		...
		if (Users.globalAuth.sectionLeaders.has(_optionalChain([targetUser, 'optionalAccess', _54 => _54.id]) || userid) && !demoting) {
			throw new Chat.ErrorMessage(`${name} is already a Section Leader of ${_roomsettings.RoomSections.sectionNames[section]}.`);
		} else if (!Users.globalAuth.sectionLeaders.has(_optionalChain([targetUser, 'optionalAccess', _55 => _55.id]) || userid) && demoting) {
		...
		if (!demoting) {
			...
			if (!_optionalChain([staffRoom, 'optionalAccess', _56 => _56.auth, 'access', _57 => _57.has, 'call', _58 => _58(userid)])) this.parse(`/msgroom staff,/forceroompromote ${userid},▸`);
			_optionalChain([targetUser, 'optionalAccess', _59 => _59.popup, 'call', _60 => _60(`You were appointed Section Leader of ${_roomsettings.RoomSections.sectionNames[section]} by ${user.name}.`)]);
		} else {
			...
			if (_optionalChain([staffRoom, 'optionalAccess', _61 => _61.auth, 'access', _62 => _62.getDirect, 'call', _63 => _63(userid)])  === '\u25B8') this.parse(`/msgroom staff,/roomdeauth ${userid}`);
			_optionalChain([targetUser, 'optionalAccess', _64 => _64.popup, 'call', _65 => _65(`You were demoted from Section Leader of ${_roomsettings.RoomSections.sectionNames[section]} by ${user.name}.`)]);
		}
	...
	htmldeclare(target, room, user) {
		...
		for (const u in room.users) {
			_optionalChain([Users, 'access', _66 => _66.get, 'call', _67 => _67(u), 'optionalAccess', _68 => _68.sendTo, 'call', _69 => _69(
				room,
	...
	forcerename(target, room, user) {
		...
		const targetID = _optionalChain([targetUser, 'optionalAccess', _70 => _70.id]) || toID(targetUsername);
		...
		if (_optionalChain([room, 'optionalAccess', _72 => _72.roomid]) !== 'staff') {
			if (_optionalChain([room, 'optionalAccess', _73 => _73.roomid, 'access', _74 => _74.startsWith, 'call', _75 => _75('help-')])) {
				this.addModAction(`${_optionalChain([targetUser, 'optionalAccess', _76 => _76.name]) || targetID} ${forceRenameMessage}`);
			} else {
				this.privateModAction(`${_optionalChain([targetUser, 'optionalAccess', _77 => _77.name]) || targetID} ${forceRenameMessage}`);
			}
		}
		const roomMessage = this.pmTarget ? `<PM:${this.pmTarget.id}>` :
			room && room.roomid !== 'staff' ? `«<a href="/${room.roomid}" target="_blank">${room.roomid}</a>» ` :
			'';
		const rankMessage = _optionalChain([targetUser, 'optionalAccess', _78 => _78.getAccountStatusString, 'call', _79 => _79()]) || "";
		Rooms.global.notifyRooms(
			['staff'],
			`|html|${roomMessage}` + _lib.Utils.html`<span class="username">${_optionalChain([targetUser, 'optionalAccess', _80 => _80.name]) || targetID}</span> ${rankMessage} ${forceRenameMessage}`
		);

		_optionalChain([targetUser, 'optionalAccess', _81 => _81.resetName, 'call', _82 => _82(true)]);
	...
	async namelock(target, room, user, connection, cmd) {
		...
		if (_optionalChain([targetUser, 'optionalAccess', _83 => _83.namelocked]) && !week) {
			return this.errorReply(`User '${targetUser.name}' is already namelocked.`);
		}
		const {privateReason, publicReason} = this.parseSpoiler(reason);
		const reasonText = publicReason ? ` (${publicReason})` : `.`;
		this.privateGlobalModAction(`${_optionalChain([targetUser, 'optionalAccess', _84 => _84.name]) || userid} was ${week ? 'week' : ''}namelocked by ${user.name}${reasonText}`);
		...
		// Automatically upload replays as evidence/reference to the punishment
		if (_optionalChain([room, 'optionalAccess', _85 => _85.battle])) this.parse('/savereplay forpunishment');
	...
	unnamelock(target, room, user) {
		...
		const targetUser = Users.get(target);
		let reason = '';
		if (_optionalChain([targetUser, 'optionalAccess', _86 => _86.namelocked])) {
	...
	async groupchatban(target, room, user, connection, cmd) {
		...
		for (const roomid of createdGroupchats) {
			...
			const participants = _optionalChain([targetRoom, 'access', _87 => _87.warnParticipants, 'optionalCall', _88 => _88(
