...
async function rebuild(context, force) {
		...
		try {
			...
			if (target === 'all') {
				...
				for (const manager of processManagers.slice()) {
					if (
						manager.filename.startsWith(_lib.FS.call(void 0, 'server/chat-plugins').path) ||
						manager.filename.startsWith(_lib.FS.call(void 0, '.server-dist/chat-plugins').path)
					) {
				...
				const processManagers = _lib.ProcessManager.processManagers;
				for (const manager of processManagers.slice()) {
					if (manager.filename.startsWith(_lib.FS.call(void 0, '.server-dist/modlog').path)) void manager.destroy();
				}
